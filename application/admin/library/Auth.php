...
class Auth extends \libs\Auth
{
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        $menu = $nav = '';
        $ruleList = list_to_tree($ruleList,'id','pid');
        // dump($ruleList);die;
        foreach($ruleList as $k=>$v){
            $menu .= '<li data-name="'.$v['title'].'" class="layui-nav-item">';
            if(!isset($v['_child'])&&$v['pid']==0){
                $menu .= '<a href="'.$v['url'].'" lay-tips="'.$v['title'].'" lay-direction="'.$v['id'].'"><i class="'.$v['icon'].'"></i><cite>'.$v['title'].'</cite></a>';
            }else{
                $menu .= '<a href="javascript:;" lay-tips="'.$v['title'].'" lay-direction="'.$v['id'].'"><i class="'.$v['icon'].'"></i><cite>'.$v['title'].'</cite></a>';
                foreach($v['_child'] as $key=>$value){
                    $menu .= '<dl class="layui-nav-child"><dd data-name="'.$value['title'].'"><a lay-href="'.$value['url'].'"><i class="'.$value['icon'].'"></i>'.$value['title'].'</a></dd></dl>';
                }
